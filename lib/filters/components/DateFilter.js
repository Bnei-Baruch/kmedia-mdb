...
const DateFilter = ({ t, namespace }) => {
  ...
  const stats = useSelector(state => filtersAside.getMultipleStats(state.filterStats, namespace, FN_DATE_FILTER)(datePresets));
  ...
  const updateFilter = (_preset, _from, _to) => {
    ...
    dispatch(filterSlice.actions.setFilterValues({ namespace, name: FN_DATE_FILTER, values: [r] }));
  };
