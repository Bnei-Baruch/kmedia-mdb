...
import {
  ...
  MY_NAMESPACE_SUBSCRIPTIONS, MY_NAMESPACE_NOTES
...
export const fetchNotes = createAppAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    const { data: { items = [] } } = await Api.myNotes(payload, token);
    return { namespace: MY_NAMESPACE_NOTES, items };
  }
);
...
export const addNote = createAppAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    const { data } = await Api.myNotes(payload, token, 'POST');
    return { namespace: MY_NAMESPACE_NOTES, item: data };
  }
);

export const editNote = createAppAsyncThunk(
  'notes/edit',
  async (payload, thunkAPI) => {
    const state = thunkAPI.getState();
    ...
    const { data } = await Api.myNotes(payload, token, 'PUT');
    return { namespace: MY_NAMESPACE_NOTES, item: data, changeItems: payload.changeItems };
  }
);

export const removeNote = createAppAsyncThunk(
  'notes/remove',
  async (payload, thunkAPI) => {
    const state = thunkAPI.getState();
    ...
    const { id } = payload;
    await Api.myNotes({ id }, token, 'DELETE');
    return { id };
  }
);
