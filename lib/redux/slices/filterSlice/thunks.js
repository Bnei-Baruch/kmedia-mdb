...
import { selectors as settingsSelectors } from '../settingsSlice/settingsSlice';
import { selectors as searchSelectors } from '../searchSlice/searchSlice';
import { filterStats } from './filterStatsSlice';
...
export const fetchStats = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    thunkAPI.dispatch(filterStats.actions.receiveLocationsStats({ locations, namespace, isPrepare }));

    if (filterParams.with_languages) {
      fetchLanguageStat({ ...filterParams }, namespace, dataL.languages, isPrepare, countCU, countC, thunkAPI);
    }
 ...
 */
async function fetchLanguageStat(params, namespace, dataL = {}, isPrepare, countCU, countC, thunkAPI) {
  setAllStatParamsFalse(params);
  ...
  thunkAPI.dispatch(filterStats.actions.receiveSingleTypeStats({ dataCU, dataC, dataL, namespace, isPrepare, fn: FN_LANGUAGES }));
}
...
export const fetchElasticStat = createAsyncThunk(
