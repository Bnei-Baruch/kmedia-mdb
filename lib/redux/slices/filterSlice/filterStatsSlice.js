...
import { fetchStats, fetchElasticStat } from '@/lib/redux/slices/filterSlice/thunks';
...
const onReceiveSingleTypeStats = (draft, { payload }) => {
  const { dataCU = {}, dataC = {}, dataL = {}, namespace, isPrepare, fn } = payload;
...
const onFetchStatsFailure = (draft, action) => {
  const { error, meta } = action;
  const ns              = meta.arg.namespace;
  if (!draft[ns]) draft[ns] = {};
  draft[ns].wip         = false;
  draft[ns].err         = error.message;
  draft[ns].needRefresh = false;
};

const onFetchElasticStatsFailure = (draft, action) => {
  const { error, meta } = action;
  const ns              = meta.arg.namespace;
  draft[ns].wip = false;
  draft[ns].err = error.message;
...
const onPending = (draft, { meta }) => {
  const { namespace } = meta.arg;
  if (!draft[namespace])
    draft[namespace] = {};
  draft[namespace].wip         = true;
  draft[namespace].needRefresh = true;
  draft[namespace].err         = null;
};

export const filterStats = createSlice({
  name: 'filterStats',
  initialState: {},
  reducers: {
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchStats.pending, onPending);
    builder.addCase(fetchStats.fulfilled, onFetchStatsSuccess);
    builder.addCase(fetchStats.rejected, onFetchStatsFailure);
    ...
    builder.addCase(HYDRATE, (state, action) => {
      console.log('filterStats HYDRATE', { ...action.payload.filterStats });
