...
import {
  ...
  fetchCollection,
  fetchWindow, fetchDatepickerCO, fetchLatestLesson, countCU
} from '@/lib/redux/slices/mdbSlice/thunks';
...
const onReceiveCollections = (state, { payload }) => {
  const items = payload || [];
...
const onReceiveContentUnits = (state, { payload }) => {
  const items = payload.content_units || [];
...
const onFetchWindow = (state, action) => {
  ...
  state.cWindow = { id, data: (data.collections || []).map(x => x.id) };
};
...
const onFetchDatepickerCO = (state, action) => {
  ...
  state.datepickerCO = sorted[0]?.id;
};

const onNullDatepickerCO = state => {
  state.datepickerCO = null;
};

const onCountCU = (state, action) => {
  if (!state.countCU[action.payload.namespace])
...
export const mdbSlice = createSlice({
  name: 'mdb',
  initialState,
  reducers: {
    receiveLabels: onReceiveLabels,
    receivePersons: onReceivePersons,
    receiveCollections: onReceiveCollections,
    receiveContentUnits: onReceiveContentUnits,
    nullDatepickerCO: onNullDatepickerCO
  },
  extraReducers: (builder) => {
    builder.addCase(HYDRATE, (state, action) => {
      return { ...state, ...action.payload.mdb, };
    });

    builder.addCase(fetchUnit.fulfilled, (state, action) => {
      setStatus(state, action);
      onReceiveContentUnits(state, { payload: { content_units: [action.payload.data] } });
    });
    builder.addCase(fetchUnit.rejected, setStatus);

    builder.addCase(fetchUnitsByIDs.fulfilled, (state, action) => {
      setStatus(state, action);
      onReceiveContentUnits(state, { payload: action.payload });
    });
    builder.addCase(fetchUnitsByIDs.rejected, setStatus);

    builder.addCase(fetchCollectionsByIDs.fulfilled, (state, action) => {
      setStatus(state, action);
      onReceiveCollections(state, { payload: action.payload.collections });
    });

    builder.addCase(fetchCollectionsByIDs.rejected, setStatus);

    builder.addCase(fetchCollection.fulfilled, (state, action) => {
      setStatus(state, action);
      onReceiveCollections(state, { payload: action.payload.collections });
    });
    builder.addCase(fetchCollection.rejected, setStatus);

    builder.addCase(fetchWindow.fulfilled, (state, action) => {
      setStatus(state, action);
      onFetchWindow(state, action);
      onReceiveCollections(state, { payload: action.payload.data?.collections });
    });
    builder.addCase(fetchWindow.rejected, setStatus);

    builder.addCase(fetchDatepickerCO.fulfilled, (state, action) => {
      setStatus(state, action);
      onFetchDatepickerCO(state, action);
      onReceiveCollections(state, { payload: action.payload.collections });
    });
    builder.addCase(fetchDatepickerCO.rejected, setStatus);

    builder.addCase(fetchLatestLesson.fulfilled, (state, action) => {
      setStatus(state, action);
      onReceiveCollections(state, { payload: [action.payload] });
      state.lastLessonId = action.payload.id;
    });
    builder.addCase(fetchLatestLesson.rejected, setStatus);

    builder.addCase(countCU.fulfilled, onCountCUSuccess);
  }
});
