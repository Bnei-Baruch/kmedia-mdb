...
import { CT_DAILY_LESSON, CT_SPECIAL_LESSON, MY_NAMESPACE_HISTORY } from '@/src/helpers/consts';
...
export const fetchUnit = createAppAsyncThunk(
  'mdb/fetchUnit',
...
export const fetchUnitsByIDs = createAppAsyncThunk(
  'mdb/fetchUnitsByIDs',
  async ({ payload }, thunkAPI) => {
    const state            = thunkAPI.getState();
    const uiLang           = settings.getUILang(state.settings);
...
export const fetchCollectionsByIDs = createAppAsyncThunk(
...
export const fetchCollection = createAppAsyncThunk(
  'mdb/fetchCollection',
  async (payload, thunkAPI) => {
    const state            = thunkAPI.getState();
    const uiLang           = settings.getUILang(state.settings);
...
export const fetchWindow = createAppAsyncThunk(
  'mdb/fetchWindow',
  async (payload, thunkAPI) => {
    const state            = thunkAPI.getState();
    const uiLang           = settings.getUILang(state.settings);
...
export const fetchDatepickerCO = createAppAsyncThunk(
...
export const fetchLatestLesson = createAppAsyncThunk(
...
export const fetchSQData = createAppAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    const args     = { ui_language: uiLang, content_languages: contentLanguages };
    const { data } = await Api.sqdata(args);

    /*
    sourcesSlice.actions.receiveSources({ sources: data.sources, uiLang });
    tags.receiveTags(data.tags);
    publications.receivePublishers(data.publishers);
    publicationsSlice.actions.receivePersons(data.persons);
*/
    return true;
  }
);

export const countCU = createAppAsyncThunk(
  'mdb/countCU',
  async (payload) => {
...
export const fetchLabels = createAppAsyncThunk(
  'mdb/fetchLabels',
  async (payload, thunkAPI) => {
    const state            = thunkAPI.getState();
    const uiLang           = settings.getUILang(state.settings);
