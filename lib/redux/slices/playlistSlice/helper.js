...
import { getQuery } from '@/src/helpers/url';
import MediaHelper from '../../../../src/helpers/media';
import { isEmpty, physicalFile } from '@/src/helpers/utils';

export const restorePreferredMediaType = () => localStorage.getItem('@@kmedia_player_media_type') || MT_VIDEO;
...
const calcAvailableMediaTypes = (unit, language) => {
  ...
  return Array.from(unit.files.reduce((acc, val) => {
    if ((val.language === language || language === 'none') && isPlayable(val)) {
      acc.add(val.type);
    }
...
export const calcAvailableLanguages = unit => {
  ...
  return Array.from(unit.files
    .filter(f => f.type === MT_VIDEO || f.type === MT_AUDIO)
    .reduce((acc, val) => {
      const { language = 'none' } = val;
      if (language) {
        acc.add(language);
      }
      return acc;
...
export const playableItem = (unit, preImageUrl) => {
  ...
  let resp = {
    id: unit.id,
    name: unit.name ?? null,
    properties: unit.properties ?? null,
    preImageUrl,
    mtByLang: {},
    qualityByLang: {}
  };
  ...
  const subtitles = unit.files.filter(f => f.type === MT_SUBTITLES).map(f => ({
    src: physicalFile(f), language: f.language
  })) || [];
  ...
  const files         = unit.files.filter(isPlayable).map(f => ({
    ...f,
    src: physicalFile(f, true),
    language: f.language || 'none'
  }));
  const filesByLang   = languages.reduce((acc, l) => ({ ...acc, [l]: files.filter(f => f.language === l) }), {});
  ...
  return {
    ...resp, languages, mtByLang, filesByLang, qualityByLang, files, subtitles,
  };
};
...
export const playlist = collection => {
  ...
  const { content_units: units = [], name = null, content_type } = collection;
...
//query utilities
export const getMediaTypeFromQuery = query => {
  const mt = (query.mediaType || '').toLowerCase();
  return [MT_VIDEO, MT_AUDIO].includes(mt) ? mt : null;
};

export const getLanguageFromQuery = query => query.shareLang || query.language || '';

export const getActivePartFromQuery = (query, def = 0) => {
  const p = query.ap ? parseInt(query.ap, 10) : def;
