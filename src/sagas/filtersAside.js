...
export function* fetchStat(action) {
  ...
  //need when have filter by base param (for example filter by topics on the topic page)
  ...
  try {
    ...
    if (isFilteredByBase) {
      ...
      const { data: dataCUPart } = yield call(Api.unitsStats, paramsPart);
      const { data: dataLPart }  = yield call(Api.labelsStats, paramsPart);
