...
const buildResults = (query, options) => {
  const results = {suggest: {title_suggest: [{offset: 0, length: 1, text: query, options: [{text: 'one', _source: {title: 'one is the best'}}]}]}};
  results.suggest.title_suggest[0].options = options.map(o => ({text: o.text, _source: {result_type: o.result_type, title: o.title}}));
  return results;
}
