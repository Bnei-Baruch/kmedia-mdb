...
import { MY_NAMESPACE_HISTORY, MY_NAMESPACE_LIKES, namespace } from '../../../helpers/consts';

const HistoryAndLikes = ({ pageSize = 8, pageNo = 1, t, namespace }) => {
  ...
  if (namespace === MY_NAMESPACE_HISTORY)
    return <Template items={items} namespace={namespace} />;
  if (namespace === MY_NAMESPACE_LIKES)
    return <Template items={items} namespace={namespace} />;
  return null;
};

export default withNamespaces()(HistoryAndLikes);
