...
const ProgramPage = ({ t }) => {
  ...
  const { collections: { [cid]: wip = false } } = useSelector(state => mdb.getWip(state.mdb));
  const { collections: { [cid]: err = null } }  = useSelector(state => mdb.getErrors(state.mdb));

  const { items, total } = useSelector(state => lists.getNamespaceState(state.lists, namespace)) || {};
      ...
      <Container className="padded" fluid>
        ...
        <Grid divided>
          ...
          <Grid.Column width="12">
            ...
            {items?.map((id, i) => <ItemOfList id={id} ccu={collection} key={i} />)}
            <Divider fitted />
