...
const getRelevantCollectionId = (unit) => {
  if (unit.collections) {
    ...
    const relevantCollection = collections.find(collection => collection.content_type === CT_ARTICLES);
...
export default connect(
  (state, ownProps) => {
    ...
    return {
      ...
      wip: selectors.getWip(state.publications).collections[collectionID],
      errors: selectors.getErrors(state.publications).collections[collectionID],
    };
  },
