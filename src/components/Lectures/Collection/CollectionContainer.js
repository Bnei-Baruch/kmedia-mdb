...
class LectureCollectionContainer extends Component {
  static propTypes = {
...
function mapState(state, props) {
  ...
  const wipMap      = selectors.getWip(state.lectures);
  const errMap      = selectors.getErrors(state.lectures);
  const paging      = selectors.getCollectionPaging(state.lectures);
  ...
  return {
    ...
    isFiltersHydrated: filters.getIsHydrated(state.filters, 'lectures-collection'),
  };
}
...
export default connect(mapState, mapDispatch)(LectureCollectionContainer);
