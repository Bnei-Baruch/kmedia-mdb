  ...
  render() {
    ...
    const tagLinks = Array.from(intersperse(
      (tags || []).map((x) => {
        ...
        return <Link key={x} to={{ pathname: '/lectures', search: urlSearchStringify(query) }}>{display}</Link>;
      }), ', '));
        ...
        <List>
          {
            tagLinks.length === 0 ?
              null :
              <List.Item>
                <strong>{t('lectures.cu.info.tags')}:</strong>
                &nbsp;{tagLinks}
              </List.Item>
          }
          {
            sourcesLinks.length === 0 ?
              null :
              <List.Item>
                <strong>{t('lectures.cu.info.sources')}:</strong>
                &nbsp;{sourcesLinks}
              </List.Item>
          }
        </List>
