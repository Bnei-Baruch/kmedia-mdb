...
const getFilesByLanguage = (files = [], contentLanguage, uiLanguage) => {
  ...
  if (images.length > 0) {
    const image = fallbackImage(images, contentLanguage, uiLanguage);
    groups.forEach(byType => {
      if (!byType.has(MT_IMAGE)) {
        byType.set(MT_IMAGE, image);
      }
    });
  }
...
const fallbackImage = (images, contentLanguage, uiLanguage) => {
...
const getDerivedFilesByContentType = (units, contentLanguage, uiLanguage) => {
  ...
  return Object.entries(allByCT).reduce((acc, val) => {
    const [ct, files] = val;
    acc[ct]           = getFilesByLanguage(files, contentLanguage, uiLanguage);
    return acc;
  }, {});
};
...
const MediaDownloads = props => {
  const { chroniclesAppend, t, publisherById, unit, displayDivider } = props;

  const [language, setLanguage] = useState();
  //const { language, languages, groups, derivedGroups } = state;
  const { isMobileDevice } = useContext(DeviceInfoContext);

  const byType = groups.get(language) || new Map();

  let typeOverrides = getI18nTypeOverridesKey(unit);
  if (typeOverrides) {
    typeOverrides += '.';
  }
  ...
  /!*
    ...
    static getDerivedStateFromProps(props, state) {
      ...
      const groups        = getFilesByLanguage(unit.files, contentLanguage, uiLanguage);
      const languages     = [...groups.keys()];
      const language      = selectSuitableLanguage(contentLanguage, uiLanguage, languages);
      const derivedGroups = getDerivedFilesByContentType(unit.derived_units, contentLanguage, uiLanguage);
  ...
  *!/
      ...
      <Table.Row key={file.id} className="media-downloads__file" verticalAlign="top">
        ...
        <Table.Cell collapsing>
          <Popup
            ...
            trigger={(
              <CopyToClipboard text={url} onCopy={() => handleCopied(url)}>
                <Button
                  compact
                  fluid
  ...
  const getDerivedRows = (derivedGroups, language, t, typeOverrides, publisherById) => {
    ...
    if (kiteiMakorByType.size > 0) {
      derivedRows = MEDIA_ORDER.reduce((acc, val) => {
        ...
        const files = (kiteiMakorByType.get(val) || []).map(file => renderRow(file, label, t));
    ...
    if (likutimByType.size > 0) {
      derivedRows = MEDIA_ORDER.reduce((acc, val) => {
        ...
        const files = (likutimByType.get(val) || []).map(file => renderRow(file, label, t));
        return acc.concat(files);
      }, derivedRows);
    }
    ...
    if (leloMikudByType.size > 0) {
      derivedRows = MEDIA_ORDER.reduce((acc, val) => {
        ...
        const files = (leloMikudByType.get(val) || []).map(file => renderRow(file, label, t));
        return acc.concat(files);
      }, derivedRows);
    }
    ...
    if (publicationsByType.size > 0) {
      derivedRows = MEDIA_ORDER.reduce((acc, val) => {
        ...
        const files = (publicationsByType.get(val) || []).map(file => {
          ...
          return renderRow(file, `${label} - ${publisher ? publisher.name : '???'}`, t);
        });
        return acc.concat(files);
      }, derivedRows);
    }
    ...
    if (articlesByType.size > 0) {
      derivedRows = MEDIA_ORDER.reduce((acc, val) => {
        ...
        const files = (articlesByType.get(val) || []).map(file => renderRow(file, label, t));
        return acc.concat(files);
      }, derivedRows);
    }
    ...
    if (researchMaterialsByType.size > 0) {
      derivedRows = MEDIA_ORDER.reduce((acc, val) => {
        ...
        const files = (researchMaterialsByType.get(val) || []).map(file => renderRow(file, label, t));
        return acc.concat(files);
      }, derivedRows);
    }
  ...
  const getRows = (byType, t, typeOverrides) => {
    let rows;
    if (byType.size === 0) {
      rows = [
        <Table.Row key="0">
