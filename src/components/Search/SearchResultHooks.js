...
import { actions as actionsSearch } from '../../redux/modules/search';
...
const searchResultClick = (chronicles, dispatch, clickData) => (link) => {
  dispatch(actionsSearch.updateQuery({ query: '', autocomplete: false }));
  chronicles.searchSelected({...clickData, link});
}

export const SearchResultCU = withNamespaces()(({ cu, highlight = {}, clickData, hideContent = false, t }) => {
  ...
  const logo = cu.content_type === CT_ARTICLE ?
    iconByContentType(cu.content_type, t, link) :
  ...
  const props = {
    ...
    part: Number(ccu.ccuNames?.[cu.id]),
...
export const SearchResultCollection = withNamespaces()(({ c, highlight, clickData, t }) => {
  ...
  const logo = c.content_type !== CT_VIDEO_PROGRAM ? iconByContentType(c.content_type, t, link) : <UnitLogoWithDuration collectionId={c.id} width={144} />;
...
export const SearchResultIntent = withNamespaces()(({ id, name, type, index, highlight, clickData, t }) => {
  ...
  const props = {
    ...
    wip,
    err,
    items: cuItems.map(cu => <SearchResultCU cu={cu} hideContent={true} />),
    parts: total,
    click: searchResultClick(chronicles, dispatch, clickData),
  };
...
export const SearchResultSeries = withNamespaces()(({ id, type, mdbUid, clickData, t }) => {
  ...
  const wipError = WipErr({ wip: wipL || wipS, err: null, t });
