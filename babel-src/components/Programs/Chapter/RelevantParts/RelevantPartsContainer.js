...
var getRelevantCollectionId = function getRelevantCollectionId(unit) {
  if (unit.collections) {
    ...
    var relevantCollection = collections.find(function (collection) {
      return collection.content_type === _consts.CT_VIDEO_PROGRAM;
    });
...
exports.default = (0, _reactRedux.connect)(function (state, ownProps) {
  ...
  return {
    ...
    wip: _programs.selectors.getWip(state.programs).fulls[collectionID],
    errors: _programs.selectors.getErrors(state.programs).fulls[collectionID]
  };
}, function (dispatch) {
  return (0, _redux.bindActionCreators)({
    fetchCollection: _programs.actions.fetchFullProgram
  }, dispatch);
