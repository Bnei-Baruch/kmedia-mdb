...
var LessonsContainer = function (_withPagination) {
  _inherits(LessonsContainer, _withPagination);
  ...
  function LessonsContainer() {
    ...
    _classCallCheck(this, LessonsContainer);

    for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = LessonsContainer.__proto__ || Object.getPrototypeOf(LessonsContainer)).call.apply(_ref, [this].concat(args))), _this), _this.handlePageChanged = function (pageNo) {
      ...
      return _withPagination3.default.handlePageChange(_this.props);
    }, _temp), _possibleConstructorReturn(_this, _ret);
  }

  _createClass(LessonsContainer, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      _get(LessonsContainer.prototype.__proto__ || Object.getPrototypeOf(LessonsContainer.prototype), 'componentWillReceiveProps', this).call(this, nextProps);
    }
  }, {
    key: 'render',
    value: function render() {
      ...
      return _react2.default.createElement(_Lessons2.default, {
        items: items,
        wip: wip,
        err: err,
        pageNo: pageNo,
...
var mapState = function mapState(state) {
  var paging = _withPagination3.default.mapState('lessons', state, _lessons.selectors, _settings.selectors);
  return {
    items: _lessons.selectors.getItems(state.lessons).map(function (x) {
      return x[1] === _consts.CT_LESSON_PART ? _mdb.selectors.getDenormContentUnit(state.mdb, x[0]) : _mdb.selectors.getDenormCollectionWUnits(state.mdb, x[0]);
    }),
    wip: _lessons.selectors.getWip(state.lessons).list,
    err: _lessons.selectors.getErrors(state.lessons).list,
    pageNo: paging.pageNo,
    total: paging.total,
    pageSize: _settings.selectors.getPageSize(state.settings),
    language: _settings.selectors.getLanguage(state.settings),
    isFiltersHydrated: _filters.selectors.getIsHydrated(state.filters, 'lessons')
  };
};

function mapDispatch(dispatch) {
  return (0, _redux.bindActionCreators)({
    fetchList: _lessons.actions.fetchList,
    setPage: _lessons.actions.setPage
  }, dispatch);
}

exports.default = (0, _reactRedux.connect)(mapState, mapDispatch)(LessonsContainer);