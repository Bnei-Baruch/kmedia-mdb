...
var getRelevantCollectionId = function getRelevantCollectionId(unit) {
  if (unit.collections) {
    ...
    var relevantCollection = collections.find(function (collection) {
      return collection.content_type === _consts.CT_DAILY_LESSON || collection.content_type === _consts.CT_SPECIAL_LESSON;
    });
...
exports.default = (0, _reactRedux.connect)(function (state, ownProps) {
  ...
  return {
    ...
    wip: _lessons.selectors.getWip(state.lessons).fulls[collectionID],
    errors: _lessons.selectors.getErrors(state.lessons).fulls[collectionID]
  };
}, function (dispatch) {
  return (0, _redux.bindActionCreators)({
    fetchCollection: _lessons.actions.fetchFullLesson
  }, dispatch);
